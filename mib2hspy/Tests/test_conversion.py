...
class TestValidDiffractionFrameConversion(TestCase):
    ...
    def tearDown(self):
        for extension in self.hdf_extensions:
            try:
                self.valid_data_path.with_suffix(extension).unlink(missing_ok=True)
...
class TestValidDiffractionSquareStackConversion(TestCase):
    def setUp(self):
        self.microscope = MicroscopeParameters()
        self.microscope.microscope = '2100F'
        self.microscope.camera = 'Merlin'
        self.microscope.acceleration_voltage = 200000
        self.microscope.cameralength = (12, nan)
        self.microscope.scan_step_x = (16, 16.)
        self.microscope.scan_step_y = (16, 16.)

        self.converter = Converter(self.microscope)
        self.valid_data_path = Path(__file__).parent.absolute() / 'test_data/test_128x128x12_16x16nm_CL12cm.mib'
        self.x_frames = int(128)
        self.y_frames = int(128)
        self.frame_size = 256
        self.chunksize = 32
        self.new_dtype = 'uint8'
        self.old_dtype = '>u2'
    ...
    def tearDown(self):
        del self.converter._data  # Delete the data object

        # Delete any converted files
        for extension in self.hdf_extensions:
            try:
                self.valid_data_path.with_suffix(extension).unlink(missing_ok=True)
            except PermissionError as e:
        ...
        for extension in self.image_extensions:
            try:
                files = self.valid_data_path.parent.expanduser().glob(
                    self.valid_data_path.with_name(self.valid_data_path.stem + '_*' + extension).name)
                for file in files:
    ...
    def test_frames(self):
        ...
        # Check that the total number of frames is correct
        self.assertEqual(self.converter.frames, self.x_frames * self.y_frames)

        # Check that frames still yield correct answer
        self.converter.reshape(self.x_frames, self.y_frames)
        self.assertEqual(self.converter.frames, self.x_frames * self.y_frames)
    ...
    def test_nx(self):
        self.converter.read_mib(self.valid_data_path)
        self.converter.reshape(self.x_frames, self.y_frames)
        self.assertEqual(self.converter.nx, self.x_frames)

    def test_ny(self):
        self.converter.read_mib(self.valid_data_path)
        self.converter.reshape(self.x_frames, self.y_frames)
        self.assertEqual(self.converter.ny, self.y_frames)

    def test_dimension(self):
        self.converter.read_mib(self.valid_data_path)
        self.converter.reshape(self.x_frames, self.y_frames)
        self.assertEqual(self.converter.dimension, 4)
        # Test further based on the size of the signal!

    def test_ndx(self):
        self.converter.read_mib(self.valid_data_path)
        self.assertEqual(self.converter.ndx, self.frame_size)
        self.assertEqual(self.converter.ndx, self.converter.data.axes_manager[1].size)

    def test_ndy(self):
        self.converter.read_mib(self.valid_data_path)
        self.assertEqual(self.converter.ndy, self.frame_size)
        self.assertEqual(self.converter.ndy, self.converter.data.axes_manager[2].size)
    ...
    def test_scan_extent(self):
        self.converter.read_mib(self.valid_data_path)
        self.converter.reshape(self.x_frames, self.y_frames)
        self.assertIsInstance(self.converter.scan_extent, (list, tuple))
        self.assertEqual(len(self.converter.scan_extent), 4)
        self.assertTrue(all([isinstance(extent, (int, float)) for extent in self.converter.scan_extent]))
    ...
    def test_get_max_value(self):
        self.converter.read_mib(self.valid_data_path)
        self.assertEqual(self.converter.get_max_value(), self.converter.data.max(axis=[0, 1, 2]).data)

    def test_reshape(self):
        # Test various cases of reshaping. Only valid for square scans!
        for x, y in zip([self.x_frames, None, self.x_frames], [None, self.y_frames, self.y_frames]):
            self.converter.read_mib(self.valid_data_path)
            self.converter.reshape(self.x_frames, self.y_frames)

            # Check that the shape of the data is correct
            self.assertEqual(len(self.converter.data.data.shape), 4)
            self.assertEqual(self.converter.data.data.shape[0], self.x_frames)
            self.assertEqual(self.converter.data.data.shape[1], self.y_frames)
            self.assertEqual(self.converter.data.data.shape[2], self.frame_size)
            self.assertEqual(self.converter.data.data.shape[3], self.frame_size)

    def test_rechunk(self):
        self.converter.read_mib(self.valid_data_path)
        self.converter.reshape(self.x_frames, self.y_frames)
        self.converter.rechunk(self.chunksize)
        self.assertListEqual(list(self.converter.data.data.chunksize), [self.chunksize] * self.converter.dimension)
    ...
    def test_apply_calibrations(self):
        ...
        # Check values
        self.assertEqual(self.converter.data.axes_manager[0].scale, self.microscope.scan_step_x)
        self.assertEqual(self.converter.data.axes_manager[1].scale, self.microscope.scan_step_y)
        self.assertEqual(self.converter.data.axes_manager[2].scale, self.microscope.diffraction_scale)
        self.assertEqual(self.converter.data.axes_manager[3].scale, self.microscope.diffraction_scale)

        self.assertEqual(self.converter.data.metadata.Acquisition_instrument.TEM.Detector.Diffraction.camera_length,
                         self.microscope.cameralength.value)
        self.assertEqual(self.converter.data.metadata.Acquisition_instrument.TEM.Detector.beam_energy,
                         self.microscope.acceleration_voltage.value)
    ...
    def test_plot_image(self):
        ...
        fig = self.converter.plot_image(inav=(int(self.x_frames / 2), int(self.y_frames / 2)))
        self.assertIsInstance(fig, plt.Figure)
    ...
    def test_plot_vbf(self):
        ...
        self.converter.rechunk(self.chunksize)
        self.converter.apply_calibrations()
        # self.converter.data.data.compute() #Compute the signal

        # Test "box" plotting
        fig = self.converter.plot_vbf('box')
        self.assertIsInstance(fig, plt.Figure)
        # box_kwargs = [
        #     {'cx': int(self.frame_size/2)},
        #     {'cy': int(self.frame_size/2)},
        #     {'width': int(self.chunksize)},
        #     {'cx': int(self.frame_size / 2), 'cy': int(self.frame_size / 2)},
        #     {'cx': int(self.frame_size / 2), 'width': int(10)},
        #     {'cy': int(self.frame_size / 2), 'width': int(10)},
        #     {'cx': int(self.frame_size / 2), 'cy': int(self.frame_size / 2), 'width': int(10)},
        # ]
        # for box_kwarg in box_kwargs:
        #     fig = self.converter.plot_vbf('box', vbf_kwargs=box_kwarg)
        #     self.assertIsInstance(fig, plt.Figure)

        # Test "circle" plotting
        fig = self.converter.plot_vbf('circle')
        self.assertIsInstance(fig, plt.Figure)
        # circle_kwargs = [
        #     {'cx': int(self.frame_size / 2)},
        #     {'cy': int(self.frame_size / 2)},
        ...
        #     {'r_inner': int(0)},
        #     {'cx': int(self.frame_size / 2), 'cy': int(self.frame_size / 2)},
        #     {'cx': int(self.frame_size / 2), 'r': int(10)},
        #     {'cx': int(self.frame_size / 2), 'r_inner': int(0)},
        #     {'cy': int(self.frame_size / 2), 'r': int(10)},
        #     {'cy': int(self.frame_size / 2), 'r_inner': int(0)},
        #     {'cx': int(self.frame_size / 2), 'cy': int(self.frame_size / 2), 'r': int(10)},
        #     {'cx': int(self.frame_size / 2), 'cy': int(self.frame_size / 2), 'r_inner': int(0)},
        #     {'cx': int(self.frame_size / 2), 'r': int(10), 'r_inner': int(0)},
        #     {'cy': int(self.frame_size / 2), 'r': int(10), 'r_inner': int(0)},
        #     {'cx': int(self.frame_size / 2), 'cy': int(self.frame_size / 2), 'r': int(10), 'r_inner': int(0)},
        # ]
        # for circle_kwarg in circle_kwargs:
        #     fig = self.converter.plot_vbf('circle', vbf_kwargs=circle_kwarg)
        #     self.assertIsInstance(fig, plt.Figure)
        self.fail()

    def test_get_square_vbf(self):
        self.skipTest('Test is covered by test_get_VBF()')
        self.converter.read_mib(self.valid_data_path)
        self.converter.reshape(self.x_frames, self.y_frames)
        self.converter.rechunk(self.chunksize)
        self.converter.apply_calibrations()
        #self.converter.data.compute()

        # List comprehension is not a huge gain and reduces readability - use for-loop instead
        # vbfs = [[[self.converter.get_square_vbf(cx, cy, width) for width in (None, 10)] for cy in (None, 0.0, int(self.frame_size/2))] for cx in (None, 0.0, int(self.frame_size/2))]

        vbfs = []
        for width in (None, 10):
            for cy in (None, 0.0, int(self.frame_size / 2)):
                for cx in (None, 0.0, int(self.frame_size / 2)):
                    vbfs.append(self.converter.get_square_vbf(cx, cy, width))

        #[[self.assertTrue(all(abs(abs(vbf1 - vbf2)/vbf2) < 1E-10)) for vbf2 in vbfs] for vbf1 in vbfs] #Check that the answers are very close to eachothers
    ...
    def test_get_circular_vbf(self):
        ...
        vbfs = []
        for r_inner in (None, 0.0, int(0)):
            for r in (None, 10 * self.converter.data.axes_manager[-1].scale, 10):
                for cy in (None, 0.0, int(self.frame_size / 2)):
                    for cx in (None, 0.0, int(self.frame_size / 2)):
                        vbfs.append(self.converter.get_circular_VBF(cx, cy, r, r_inner))

        [[self.assertTrue(all(abs(abs(vbf1 - vbf2) / vbf2) < 1E-10)) for vbf2 in vbfs] for vbf1 in
         vbfs]  # Check that the answers are very close to eachothers
    ...
    def test_get_vbf(self):
        ...
        self.converter.rechunk(self.chunksize)
        self.converter.apply_calibrations()
        self.converter.downsample('bool')
        self.converter.data.compute()
        ...
        for width in (None, 10):
            for cy in (None, 0.0, int(self.frame_size / 2)):
                for cx in (None, 0.0, int(self.frame_size / 2)):
                    try:
                        vbfs.append(self.converter.get_VBF('box', cx=cx, cy=cy, width=width))
                    except Exception as e:
                        print('Centre: ({cx}, {cy})\nWidth: {width}'.format(cx=cx, cy=cy, width=width))
                        raise e
        ...
        for r_inner in (None, 0.0, int(0)):
            for r in (None, 10 * self.converter.data.axes_manager[-1].scale, 10):
                for cy in (None, 0.0, int(self.frame_size / 2)):
                    for cx in (None, 0.0, int(self.frame_size / 2)):
                        try:
                            vbfs.append(self.converter.get_VBF('circle', cx=cx, cy=cy, r=r, r_inner=r_inner))
                        except Exception as e:
                            print('Centre: ({cx}, {cy})\nWidth: {width}'.format(cx=cx, cy=cy, width=width))
                            raise e
        [[self.assertTrue(all(abs(abs(vbf1 - vbf2) / vbf2) < 1E-10)) for vbf2 in vbfs] for vbf1 in
    ...
    def test_write(self):
        ...
        for extension in self.hdf_extensions:
            ...
            # Data values
            self.assertAlmostEqual(self.converter.data, s)
            ...
            self.assertListEqual(list(s.data.chunks), list(self.converter.data.data.chunks))
            self.assertListEqual(list(s.data.chunksize), list(self.converter.data.data.chunksize))
            self.assertListEqual(list(s.data.chunksize), [self.chunksize] * self.converter.dimension)
            ...
            #WIP - must update the calibration table to include values for the scan step values!
            self.assertEqual(s.axes_manager[0].scale, self.microscope.scan_step_x.value)  # Check this especially!
            self.assertEqual(s.axes_manager[1].scale, self.microscope.scan_step_y.value)  # Check this especially!
            self.assertEqual(s.axes_manager[2].scale, self.microscope.diffraction_scale.value)  # Check this especially!
            self.assertEqual(s.axes_manager[3].scale, self.microscope.diffraction_scale.value)  # Check this especially!

            # Check metadata [WIP] Tricky to compare dictionaries; for some reason the order of the elements matter?
            self.maxDiff = None
            # self.assertDictEqual(s.metadata.as_dictionary(), self.converter.data.metadata.as_dictionary())
            # self.assertDictEqual(s.original_metadata.as_dictionary(), self.converter.data.original_metadata.as_dictionary())
        ...
        [self.converter.write(extension, overwrite=True) for extension in self.image_extensions]
        for extension in self.image_extensions:
            files = self.valid_data_path.parent.expanduser().glob(self.valid_data_path.with_name(self.valid_data_path.stem + '_*' + extension).name)
            for file in files:
                print('Checking file {file}...'.format(file=file))
                s = plt.imread(file)
                self.assertEqual(s.shape[0], self.figsize_inches * self.dpi)
                self.assertEqual(s.shape[1], self.figsize_inches * self.dpi)
                print('\tOK.\n')
    ...
    def test_save_plot(self):
        self.skipTest('This test should be covered by test_write().')
